...
le_pack_size = joblib.load('le_pack_size.pkl')
...
def predict_rf_demand():
    medicine_name = input("Enter the medicine name: ").strip()
    target_month_str = input("Enter the target month (e.g., May 2025): ").strip()

    try:
        target_month = pd.to_datetime(target_month_str).to_period('M')
    except:
        print("Invalid month format. Use format like 'May 2025'")
        return

    past_month1 = target_month - 1
    past_month2 = target_month - 2

    # Load all records for the medicine
    ...
    # Group by month
    monthly_data = data.groupby(['medicine_name', 'manufacturer_name', 'pack_size_label', 'month']) \
                       .agg({'order_quantity': 'sum', 'price': 'mean'}) \
                       .reset_index()
    ...
    price = past1_row['price'].values[0]
    ...
    X_input = pd.DataFrame([[med_enc, man_enc, pack_enc, price, past1_qty, past2_qty]],
